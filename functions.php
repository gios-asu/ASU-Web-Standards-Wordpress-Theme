...
function asu_webstandards_scripts()
{
  ...
  $asu_web_standards_version = '1.1.0';
  ...
  // ASU Header Javascript
  wp_register_script('asu-header', get_template_directory_uri() . '/assets/asu-header/js/asu_header.min.js', array(), $asu_header_version, true);
  ...
  wp_register_style('asu-header-media-css', get_template_directory_uri() . '/assets/asu-header/css/asu_header_media.min.css', array(), $asu_header_version, 'all');
...
function bybe_crumb_fix($output, $crumb)
{
  if (is_array($crumb) && $crumb !== array()) {
    if (
      false !== strpos($output, '<span class="breadcrumb_last"') ||
